...
author: fan
type: post
date: 2019-10-30T06:04:04+00:00
url: /go2-2/
categories:
  - Go
...
Go于2009年11月10日发布。不到24小时后，我们看到了[关于泛型][2]的[第一条评论][2]。（该评论还提到了我们在2010年初以panic和recover的形式添加到语言中的情况。）
...
用 [Jazayeri等人的话来说][3]：泛型编程能够以通用类型的形式表示函数和数据结构。
...
<pre><code class="line-numbers">func ReverseInts(s []int) {
    first := 0
    last := len(s)
    for first &lt; last {
...
<pre><code class="line-numbers">func ReverseStrings(s []string) {
    first := 0
    last := len(s) - 1
    for first &lt; last {
        s[first], s[last] = s[last], s[first]
...
但泛型并非来自[Big Rock Candy Mountain][4]，那里每天阳光照射在[柠檬水泉上][5]。每一种语言变化都有成本。毫无疑问，向Go添加泛型将使语言更复杂。与语言的任何变化一样，我们需要谈论最大化利益并最大限度地降低成本。
...
在今年的Gophercon Robert Griesemer和我发布了[一个设计草案][6]，为Go添加泛型。有关详细信息，请参阅草案。我将在这里讨论一些要点。

这是此设计中的通用反向函数。

<pre><code class="line-numbers">func Reverse (type Element) (s []Element) {
    first := 0
    last := len(s) - 1
    for first &lt; last {
        s[first], s[last] = s[last], s[first]
...
<pre><code class="line-numbers">func ReverseAndPrint(s []int) {
    Reverse(int)(s)
    fmt.Println(s)
}
</code></pre>
...
<pre><code class="line-numbers">func IndexByte (type T Sequence) (s T, b byte) int {
    for i := 0; i &lt; len(s); i++ {
        if s[i] == b {
            return i
        }
...
<pre><code class="line-numbers">contract Sequence(T) {
    T string, []byte
}
</code></pre>
...
任何记得我们在[Gophercon 2018上展示过的设计][7]的人 都会发现这种签订合约的方式要简单得多。我们得到了很多关于合约过于复杂的早期设计的反馈，我们已经尝试将其考虑在内。新合约的编写，阅读和理解都要简单得多。
...
<pre><code class="line-numbers">func ToStrings (type E Stringer) (s []E) []string {
    r := make([]string, len(s))
    for i, v := range s {
        r[i] = v.String()
    }
...
<pre><code class="line-numbers">contract Stringer(T) {
    T String() string
}
</code></pre>
...
<pre><code class="line-numbers">type Graph (type Node, Edge G) struct { ... }
...
<pre><code class="line-numbers">func Min (type T Ordered) (a, b T) T {
    if a &lt; b {
        return a
    }
    return b
...
<pre><code class="line-numbers">contract Ordered(T) {
...
<pre><code class="line-numbers">func Min (type T contracts.Ordered) (a, b T) T {
    if a &lt; b {
        return a
    }
    return b
...
<pre><code class="line-numbers">type Tree (type E) struct {
    root    *node(E)
...
<pre><code class="line-numbers">func New (type E) (cmp func(E, E) int) *Tree(E) {
...
<pre><code class="line-numbers">func (t *Tree(E)) find(v E) **node(E) {
    pn := &t.root
    for *pn != nil {
...
<pre><code class="line-numbers">func (t *Tree(E)) Contains(v E) bool {
    return *t.find(e) != nil
}
</code></pre>
...
<pre><code class="line-numbers">func (t *Tree(E)) Insert(v E) bool {
    pn := t.find(v)
    if *pn != nil {
        return false
    }
...
<pre><code class="line-numbers">var intTree = tree.New(func(a, b int) int { return a - b })
...
Robert Griesemer编写了一个[初步的CL][8]，修改了go/types包。这允许测试使用泛型和合约的代码是否可以键入检查。它现在还不完整，但它主要适用于单个包，我们将继续努力。
...
译文：https://github.com/llgoer/go-generics
 ...
 [1]: https://github.com/llgoer/go-generics/blob/master/README-en.md
 [2]: https://groups.google.com/d/msg/golang-nuts/70-pdwUUrbI/onMsQspcljcJ
 [3]: https://www.dagstuhl.de/en/program/calendar/semhp/?semnr=98171
 [4]: https://mainlynorfolk.info/folk/songs/bigrockcandymountain.html
 [5]: http://www.lat-long.com/Latitude-Longitude-773297-Montana-Lemonade_Springs.html
 [6]: https://github.com/golang/proposal/blob/master/design/go2draft-contracts.md
 [7]: https://github.com/golang/proposal/blob/4a530dae40977758e47b78fae349d8e5f86a6c0a/design/go2draft-contracts.md
 [8]: https://golang.org/cl/187317